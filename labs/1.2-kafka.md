...
```bash
    #  be sure to be in ~/dev directory
    $   cd   ~/dev
...
```bash
  $   echo -e "\n\ndelete.topic.enable=true \n" >> ~/dev/kafka/config/server.properties
```

Start Kafka
```bash
    $   JMX_PORT=9999  ~/dev/kafka/bin/kafka-server-start.sh -daemon   ~/dev/kafka/config/server.properties
```
